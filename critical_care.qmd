...
::: {.callout-note title="Importance of Illness Severity Assessment"}
- Resource allocation
- Hospital administrative policies
- Assess quality of care
:::

::: {.callout-important title="Red Flag"}
Patient with RR > 22/min, GCS Decreased, atau SBP < 100 mmHg has high mortality.
:::
...
As the current guideline, qSOFA not recommended as the main diagnostic tools.[^1]

[^1]: Sepsis-3 Guidelines, JAMA (2016).

::: {.column-margin}
![Ilustrasi SOFA](image/qsofa.jpg)
:::
...
sofa_tbl <- 
  ...
  opt_row_striping() |>                 # Menambahkan warna selang-seling pada baris
  tab_options(
    table.width = pct(100),             # Memperlebar tabel memenuhi halaman
    data_row.padding = px(12)           # Memberi ruang napas (spasi) antar teks di dalam sel
  ) |>
  cols_align(align = "center", columns = c("0","1","2","3","4"))
```
...
$$
QO_2 = \text{Cardiac Output} \text{ (dL/min)} \times (1.39 \times \text{HB concentration} \times \text{HB\% Saturation} + 0.0031 \times PaO_2)
$$ {#eq-delivery-oxygen}

$$
QO_2 = 50 \text{ dL/min} \times (1.39 \times 15 \text{g/dL} \times 100\% + 0.0031 \times 100)
$$ 

$$
QO_2 = 50 \text{ dL/min} \times 21.6 \text{mL O2 per dL blood or CaO2} = 1058 \text{ mL O2 per min}
$$